...
async def start_monitoring(instance_id=None):
    ...
    tasks = []
    clients = []  # Lista di client creati
    ...
    try:
        for nickname, phone_number in phone_numbers.items():
            ...
            clients.append((client, client_key))
            ...
            async def run_client(nickname, phone_number, client, client_key):
                try:
                    ...
                    # Assicurati che il client venga disconnesso
                    try:
                        if client.is_connected():
                            await client.disconnect()
                            print(f"Client {nickname} disconnesso correttamente")
                    except Exception as e:
                        log_error(f"Errore durante la disconnessione del client {nickname}: {e}")
        ...
        try:
            ...
            # Non sollevare l'eccezione, continua con la pulizia
            return False
        
    except Exception as e:
        log_error(f"Errore durante l'avvio del monitoraggio: {e}")
        return False
    finally:
        # Assicurati che tutti i client vengano disconnessi
        for client, client_key in clients:
            try:
                if client.is_connected():
                    await client.disconnect()
            except Exception as e:
        ...
        # Pulisci le sessioni solo dopo aver disconnesso tutti i client
        time.sleep(2)  # Breve attesa per permettere alle disconnessioni di completarsi
        
        # Rilascia le sessioni per questa operazione
